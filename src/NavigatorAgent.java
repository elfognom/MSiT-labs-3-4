...
public class NavigatorAgent extends Agent{
    ...
    private String[] get_actions(AID request_agent, Position request_agent_position, String[] room_info){
        System.out.println("(НАВ)Так, агент у нас в [" + agentX + "; " + agentY + "]");
        System.out.print("(НАВ)До этого был в: " + request_agent_position.getX() + " | " + request_agent_position.getY() + "\n");

        int[] actions;
        ImaginaryRoom checking_room = world.getWorldGrid().get(request_agent_position);
        if (checking_room == null)
        {
            System.out.println("(ИИ)Такс, мы тут не были, сек");
        ...
        if (! Arrays.asList(room_info).contains(BUMP)){
            ...
            System.out.println("(ИИ)Ой, тут была стена? Ну сорь");
        }
        ...
        if (world.isWumpusAlive() && world.getWumpusRoomCount() > 2){
            Position wumpusPosition = world.getWumpusCoords();
...
//            System.out.println("ADVICE ACTIONS IS: " + Arrays.toString(actions));
            System.out.println("(ИИ)Ну, стоит пойти в : " + nextOkRooms[best_candidate].getX() + " | " + nextOkRooms[best_candidate].getY());
    ...
    private void updateNeighbors(Position request_agent_position) {
        ...
        if (currentRoom.getStench() == NavigatorAgent.ROOM_STATUS_TRUE){
            ...
            for (ImaginaryRoom room: roomList){
                if (room.getWumpus() == NavigatorAgent.ROOM_STATUS_NO_STATUS){
                    room.setOk(NavigatorAgent.ROOM_STATUS_POSSIBLE);
                    room.setWumpus(NavigatorAgent.ROOM_STATUS_POSSIBLE);
                }
            }
        }
        ...
        if (currentRoom.getBreeze() == NavigatorAgent.ROOM_STATUS_FALSE && currentRoom.getStench() == NavigatorAgent.ROOM_STATUS_FALSE){
            for (ImaginaryRoom room: roomList){
                ...
                room.setWumpus(NavigatorAgent.ROOM_STATUS_FALSE);
